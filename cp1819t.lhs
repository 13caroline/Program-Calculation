...
Analisando o tipo da nossa estrutura, é possível verificar que este contém um \emph{Num} inteiro, ou uma \emph{Bop}, que representa uma operação aritmética. Desta forma, dado um input, este poderá ser de um tipo ou do outro. 

\begin{code}

inExpr :: Either Int (Op,(Expr,Expr)) -> Expr
inExpr = either Num opToExpr
            where opToExpr (x,(y,z)) = Bop y x z 

\end{code}

Sabendo que o \emph{out} é a função conversa do \emph{in}, é possível, a partir de uma expressão \emph{Expr}, obter um resultado do tipo \emph{Num} ou do tipo \emph{Bop}, aplicando uma injeção à esquerda ou à direita, respetivamente.

\begin{code}

outExpr :: Expr -> Either Int (Op,(Expr,Expr))
outExpr (Num x) = Left x
outExpr (Bop y x z) = Right(x,(y,z))
...
De seguida, é definida a função \emph{calcula}, que permite calcular o valor de uma dada expressão. Para esse efeito, utiliza-se a recursividade sobre um co-produto de \emph{id}, que representa o número inteiro constante, com a função \emph{calculaAux}, que realiza a operação da expressão.

\begin{code}

calcula :: Expr -> Int
calcula = cataExpr (either id calculaAux)

calculaAux :: (Op,(Int,Int)) -> Int 
calculaAux (Op x,(y,z)) | x == "*" = y * z 
                        | x == "+" = y + z
                        | x == "-" = y - z
                        | x == "/" = div y z
...
A função \emph{show'} deve permitir gerar a representação textual de uma expressão \emph{Expr}. Deste modo, recorre-se à recursividade sobre um co-produto de \emph{prettyInt} com \emph{prettyOp}. A função \emph{prettyInt} aplica a função \emph{show}, que converte um inteiro para uma string legível. Por outro lado, a função \emph{prettyOp} converte uma operação numa string legível.                  

\begin{code}
show' :: Expr -> String 
show'= cataExpr (either prettyInt prettyOp)

prettyInt :: Int -> String 
prettyInt x = show x 

prettyOp :: (Op,(String,String)) -> String 
prettyOp (Op y,(x,z)) = "(" ++ x ++ " " ++ y ++ " " ++ z ++ ")"
\end{code}
...
Na \emph{compile}, pretende-se gerar um código posfixo para uma máquina elementar de stack. Para que tal seja possível, é necessário converter a string dada para uma expressão \emph{Expr}. A função \emph{converteString} recorre à \emph{readExp}, de modo a criar uma lista de duplos. Essa lista irá conter no primeiro elemento da cabeça a expressão desejada. Após esta conversão, vai ser aplicado recursivamente o co-produto de \emph{inteiro}, devolvendo a representação desejada para um número inteiro, com \emph{op}, que devolve a representação desejada para uma operação.

\begin{code}
compile :: String -> Codigo
compile = cataExpr (either inteiro op) . converteString

converteString :: String -> Expr
converteString = fst . head . readExp 
...
Com esta função, obtém-se todas as folhas de um elemento do tipo X a b. A função usa recursivamente o co-produto do \emph{singl}, que cria uma lista com um só elemento, e \emph{conc.snd}.   

\begin{code}
collectLeafs :: X a b -> [a]
collectLeafs = cataL2D(either singl (conc.snd))
\end{code}

\subsubsection{Dimen}
A função \emph{dimen} tem como objetivo calcular as dimensões de um elemento do tipo X Caixa Tipo, com base na suas caixas e o posicionamento relativo das últimas.

\begin{code}
dimen :: X Caixa Tipo -> (Float, Float)
dimen (Unid caixa) = (fromIntegral (p1(p1(caixa))),fromIntegral (p2(p1(caixa))))
dimen (Comp tp c1 c2)
            | tp==V = ( max (p1 (d c1)) (p1 (d c2) + ((p1 (d c1))/2)) , (p2 (d c1)) + (p2 (d c2)) )
            | tp==Ve = ( max (p1 (d c1)) (p1 (d c2)) , (p2 (d c1)) + (p2 (d c2)) )
            | tp==H = ( (p1 (d c1)) + (p1 (d c2)) , max (p2 (d c1)) (p2 (d c2) + ((p2 (d c1))/2)) )
            | tp==Hb = ( (p1 (d c1)) + (p1 (d c2)) , max (p2 (d c1)) (p2 (d c2)) )
            | tp==Ht || tp==Vd = ( (p1 (d c1)) + (p1 (d c2)) , (p2 (d c1)) + (p2 (d c2)) )
              where
                d = dimen
\end{code}

\subsubsection{CalcOrigins}
A função \emph{calcOrigens} visa calcular as coordenadas iniciais das caixas no plano. Para que tal se torne possível, é necessária a criação de uma função auxiliar \emph{calc}, que calcula a origem da próxima caixa, com base no tipo, origem e dimensões da caixa atual. 
...
calc tp o pos | tp==V = ( p1(o) + (p1(pos)/2) , p2(o) + p2(pos) )
              | tp==Ve = ( p1(o) , p2(o) + p2(pos) )
              | tp==H = ( p1(o) + p1(pos) , p2(o) + (p2(pos)/2) )
              | tp==Hb = ( p1(o) + p1(pos) , p2(o) )
              | tp==Ht || tp==Vd = ( p1(o) + p1(pos) ,  p2(o) + p2(pos) )
\end{code}

\subsubsection{AgrupaCaixas}
Com a \emph{agrupacaixas}, é possível agrupar todas as caixas e respectivas origens numa só lista. Ao receber um tipo X (Caixa,Origem), transforma numa \emph{Fig}. 

\begin{code}
agrup_caixas :: X (Caixa,Origem) () -> Fig
agrup_caixas = cataL2D(either (singl.swap) (conc.snd))
\end{code}

\subsubsection{MostraCaixas}
A função \emph{mostracaixas} necessita das funções auxiliares \emph{caixasAndOrigin2Pict}, que devolve a \emph{G.Picture} que tem que ser apresentada pelo display, e \emph{cao2pAux} que, dada uma \emph{Fig}, concatena numa lista de \emph{G.Picture}.
...
cao2pAux = map k
  where
   k (o, cx) = (crCaixa o (fI (p1(p1(cx)))) (fI (p2(p1(cx)))) (p1(p2(cx))) (p2(p2(cx))))
   fI = fromIntegral
\end{code}
...
\start
    |out . in = id|
%
\just\equiv{ definição in ; Reflexão-+ }
% 
    |out . [File,Dir] = [i1,i2]|
%
\just\equiv{ Fusão-+ }
% 
    |[out . File, out . Dir]| = [i1,i2]
%
\just\equiv{ Eq-+ }
%
        |lcbr(
...
% 
\just\equiv{ introdução variáveis }
%
        |lcbr(
        (out . File) b = i1 b
        )(
        (out . Dir) (Fs b) = i2 (Fs b)
    )|
% 
\just\equiv{ Def-comp; Definição i1/i2}
%
        |lcbr(
        out (File b) = Left b
        )(
        (out . Dir) (Fs b) = Right (Fs b)
    )|
\qed
\end{eqnarray*}
...
\subsubsection{Check}
A função \emph{check} visa realizar a verificação da integridade do sistema de ficheiros, isto é, verificar que não existem identificadores repetidos dentro da mesma diretoria. Desta forma, foi construída a função auxiliar \emph{checkExtra} que agrupa numa lista todos os identificadores de um sistema de ficheiros. A função \emph{check} vai recorrer à função \emph{nr}, definida em \emph{List.hs}, após a \emph{checkExtra}, de modo a confirmar a existência de identificadores repetidos, ou não.

\begin{code}
check :: (Eq a) => FS a b -> Bool
check = nr . checkExtra
 
checkExtra :: (Eq a) => FS a b -> [a]
checkExtra (FS info) = map (p1) info
\end{code}

\subsubsection{Tar}
A função \emph{tar} recolhe o conteúdo de todos os ficheiros num arquivo indexado pelo path. 
...
\\ 
&
|g = concat . map(either (singl . (singl >< id)) (tarAux) . distr)|
}
\end{eqnarray*}
...
fichAdd::(([a],b),FS a b) ->FS a b
fichAdd (([],_),FS []) = FS []
fichAdd (([x],b),FS l) =FS ((x,File b):l)
fichAdd (((h:t),b),FS l) = FS (singl (h,Dir (fichAdd ((t,b),FS l))))

untar :: (Eq a)=> [(Path a, b)] ->FS a b
untar = joinDupDirs . cataList(either (auxUnt) fichAdd)

find :: (Eq a) => a -> FS a b -> [Path a]
find _ (FS [])  =  []
find x l  =  if (length(findAux  x (tar l)))==0 then [] 
                                                else (map (p1) (findAux  x (tar l)))

findAux :: (Eq a) => a -> [(Path a, b)] -> [(Path a,b)]
findAux = filter . (\p -> \(a,b) -> p == (last a) )

new :: (Eq a) => Path a -> b -> FS a b -> FS a b
new pt x (FS z) = untar ( (tar (FS z)) ++ [(pt,x)] );

cp :: (Eq a) => Path a -> Path a -> FS a b -> FS a b
cp path1 path2 (FS fich) = untar ( auxCP (tar (FS fich)) path1 path2 )

auxCP :: (Eq a) => [(Path a,b)] -> Path a -> Path a -> [(Path a,b)]
auxCP [] _ _ =  []
auxCP ((pt,obj):t) path1 path2 = if pt == path1
                                 then [(pt,obj)] ++ t ++ [(path2,obj)]
                                 else [(pt,obj)] ++ (auxCP t path1 path2)
